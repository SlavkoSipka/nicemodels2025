...
/**
 * Returns the first country that matches the given dial code.
 * Prefers exact match. For shared codes (e.g. +1 -> US/CA), returns the
 * canonical entry (United States for +1, United Kingdom for +44, Russia for +7).
 */
export function findByDialCode(dialCode: string): Country | undefined {
  ...
  const preferred = canonical[normalized]
  if (preferred) {
    const match = COUNTRIES.find(c => c.dialCode === normalized && c.code === preferred)
...
/**
 * Splits a stored phone string (e.g. "+41 79 123 45 67" or "+381641234567")
 * into { dialCode, number }. Tries the longest matching dial code; falls back
 * to the default and returns the entire string as the number.
 */
export function splitPhone(full: string | null | undefined): { dialCode: string; number: string } {
  const raw = (full ?? '').trim()
  if (!raw) return { dialCode: DEFAULT_DIAL_CODE, number: '' }
  if (!raw.startsWith('+')) return { dialCode: DEFAULT_DIAL_CODE, number: raw }

  const sortedDials = Array.from(new Set(COUNTRIES.map(c => c.dialCode))).sort(
    (a, b) => b.length - a.length
  )
  for (const dial of sortedDials) {
    if (raw.startsWith(dial)) {
      const rest = raw.slice(dial.length).replace(/^[\s-]+/, '')
      return { dialCode: dial, number: rest }
    }
  }
  return { dialCode: DEFAULT_DIAL_CODE, number: raw }
}

/**
 * Joins dial code + number into a single international phone string.
 * Returns an empty string if both parts are empty.
 */
