...
export async function generateMetadata({ params }: PageProps): Promise<Metadata> {
  ...
  if (!post) {
    return { title: 'Magazin', robots: { index: false, follow: false } }
  }

  const url = magazineUrl(slug)
...
export default async function MagazineArticle({ params }: PageProps) {
  ...
  const breadcrumbs = buildBreadcrumbJsonLd([
    ...
    { name: 'Magazin', path: MAGAZINE_PATH },
    { name: post.title, path: `${MAGAZINE_PATH}/${slug}` },
  ])
    ...
    <>
      ...
      <main className="mx-auto w-full max-w-3xl px-4 py-10">
        <nav className="mb-6 text-sm text-gray-500">
          <Link href={MAGAZINE_PATH} className="hover:text-rose-600">
            Magazin
          </Link>
        </nav>
