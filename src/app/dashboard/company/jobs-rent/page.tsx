...
import {
  Briefcase, Plus, Trash2, MapPin, Calendar, AlertCircle, CheckCircle, Clock, Eye, Pencil
} from 'lucide-react'
...
export default function ManageJobsRentPage() {
  ...
  const loadListings = async () => {
    ...
    const { data, error: fetchErr } = await supabase
      .from('job_listings')
      .select('id, listing_type, title, location, description, status, starts_at, expires_at, created_at')
      .eq('club_id', user.id)
      .neq('status', 'deleted')
      .order('created_at', { ascending: false })

    if (fetchErr) {
      // Table doesn't exist yet (42P01) — show empty state instead of error
      ...
      <div className="max-w-6xl mx-auto space-y-4">
        ...
        <div className="flex items-center justify-between gap-4 flex-wrap">
          <div className="flex items-center gap-2">
            ...
            <div>
              <h1 className="text-xl font-bold text-gray-900">Jobs & Rent</h1>
              <p className="text-xs text-gray-500">Manage your job and rental listings</p>
            </div>
          </div>
          <button
            onClick={() => router.push('/dashboard/company/jobs-rent/create')}
        ...
        {listings.length === 0 ? (
          <div className="bg-white border border-gray-200 rounded-lg p-10 text-center">
            ...
            <p className="text-xs text-gray-400 mb-4">Create your first job or rent listing</p>
            <button
              onClick={() => router.push('/dashboard/company/jobs-rent/create')}
                  ...
                  <div className="flex flex-col sm:flex-row">
                    ...
                    <div className="flex-1 p-4 min-w-0 flex flex-col justify-between gap-3">
                      <div>
                        ...
                        <p className="text-sm text-gray-700 line-clamp-2">{listing.description?.replace(/<[^>]*>/g, '')}</p>
                      </div>
                      ...
                      <div className="flex items-center justify-between">
                        <div className="flex items-center gap-3 text-xs text-gray-400">
                          ...
                          {listing.expires_at && (
                            <span className="flex items-center gap-1">
                              ...
                              Expires {new Date(listing.expires_at).toLocaleDateString('en-GB', { day: '2-digit', month: 'short' })}
                            </span>
                          )}
                        </div>
                        <div className="flex items-center gap-1.5">
                          ...
                          <button
                            onClick={() => router.push(`/dashboard/company/jobs-rent/edit/${listing.id}`)}
                            className="flex items-center gap-1 px-3 py-1.5 bg-brand/10 text-brand rounded-lg text-xs font-semibold hover:bg-brand/20 transition-colors"
                          >
