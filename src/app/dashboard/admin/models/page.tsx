...
export default function AdminModelsPage() {
  ...
  const handleDownloadXlsx = async () => {
    await downloadXlsx('nicemodels-models', sorted, [
      ...
      <div className="py-4 px-3 sm:py-6 sm:px-6">
        <div className="max-w-7xl mx-auto space-y-4">
          ...
          <div className="md:hidden space-y-2.5">
            {sorted.map(model => (
              <div key={model.id} className="bg-white border border-gray-200 rounded-lg p-3">
                <div className="flex items-start gap-3">
                  <Link href={`/dashboard/admin/models/${model.id}`} className="w-12 h-12 rounded-full bg-brand/10 flex items-center justify-center shrink-0 overflow-hidden relative">
                    {model.photoUrl ? (
                      <Image src={model.photoUrl} alt={model.model_details?.showname || model.username} fill sizes="48px" className="object-cover object-top" />
                    ) : (
                  ...
                  <div className="flex-1 min-w-0">
                    <div className="flex items-start justify-between gap-2">
                      <div className="min-w-0">
                        <Link href={`/dashboard/admin/models/${model.id}`} className="text-sm font-semibold text-gray-900 truncate block hover:text-brand">
                        ...
                        <a href={`mailto:${model.email}`} className="text-xs text-gray-500 hover:text-brand truncate block">{model.email}</a>
                      </div>
                    ...
                    <div className="mt-1.5 flex flex-wrap gap-x-3 gap-y-0.5 text-[11px] text-gray-500">
                      {model.model_details?.city && <span>{model.model_details.city}</span>}
                      <span>{new Date(model.created_at).toLocaleDateString()}</span>
                      {!model.onboarding_completed && <span className="text-amber-600 font-medium">{tc('incomplete')}</span>}
                    </div>
                  </div>
                </div>
                <div className="mt-2.5 pt-2.5 border-t border-gray-100 flex flex-wrap items-center gap-1.5">
                  ...
                  <button onClick={() => handleBlock(model.id, model.is_blocked)}
                    className={`flex-1 min-w-[80px] px-2.5 py-1.5 text-xs font-semibold rounded-md ${
                      model.is_blocked ? 'bg-emerald-50 text-emerald-700' : 'bg-red-50 text-red-700'
                    }`}>
