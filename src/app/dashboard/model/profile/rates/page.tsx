...
export default function RatesPage() {
  ...
  const getDurationLabel = (v: string) => durationOptions.find(o => o.value === v)?.label || v
  ...
  const RateSection = ({ title, rates, newRate, setNewRate, onAdd, onRemove }: {
    title: string; rates: Rate[]; newRate: Rate; setNewRate: (r: Rate) => void; onAdd: () => void; onRemove: (i: number) => void
  }) => (
    <div className="bg-white border border-gray-200 rounded-lg p-5">
      <p className="text-sm font-bold text-gray-800 mb-3">{title}</p>
      <div className="space-y-2.5 mb-3">
        <select value={newRate.duration} onChange={e => setNewRate({ ...newRate, duration: e.target.value })} className={inputCls + ' w-full'}>
