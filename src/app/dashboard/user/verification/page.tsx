...
export default function UserVerificationPage() {
  ...
  const t = useTranslations('dashboard.user.verification')
  ...
  useEffect(() => {
    const loadData = async () => {
      try {
        ...
        if (profile?.role !== 'user') { router.push('/dashboard'); return }
  ...
  const handleImgChange = (
    ...
    if (!allowed.includes(file.type)) { setSubmitError(t('errInvalidFile')); return }
  ...
  const handleVideoChange = (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    if (!allowed.includes(file.type)) { setSubmitError(t('errInvalidVideo')); return }
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    if (!formData.first_name || !formData.surname || !formData.date_of_birth || !formData.id_number) { setSubmitError(t('errFillRequired')); return }
    if (!idCardPhoto && !verification?.id_card_photo_path) { setSubmitError(t('errUploadIdCard')); return }
    if (!selfiePhoto && !verification?.selfie_photo_path) { setSubmitError(t('errUploadSelfie')); return }
    setSubmitting(true)
    try {
      const supabase = createClient()
      ...
      setSubmitError(e?.message || t('errSubmitFailed'))
            ...
            <div>
              ...
              <p className={`text-xs mt-0.5 ${
                ...
                {verification.status === 'approved' ? t('verifiedHint') :
                 verification.status === 'rejected' ? (verification.rejection_reason ? t('rejectedReasonPrefix', { reason: verification.rejection_reason }) : t('rejectedDefault')) :
                 t('pendingHint')}
              </p>
            </div>
          </div>
