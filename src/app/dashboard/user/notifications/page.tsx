          ...
          <div className="mb-6">
            <div className="flex flex-wrap items-center justify-between gap-3 mb-4">
              <div className="flex items-center gap-3">
                ...
                <div>
                  ...
                  {counts.unread > 0 ? (
                    <p className="text-sm text-gray-600 mt-0.5">{counts.unread} unread</p>
                  ) : (
                    <p className="text-sm text-gray-500 mt-0.5">You are all caught up</p>
                  )}
                </div>
              </div>
            ...
            <div className="flex flex-wrap items-center gap-2">
              {TABS.map(t => (
                ...
                >
                  {t.label}
                  <span className={`ml-1.5 ${tab === t.id ? 'opacity-80' : 'opacity-60'}`}>{t.count}</span>
                </button>
              ))}
            </div>
          </div>
          ...
          {filtered.length === 0 ? (
            <div className="bg-white rounded-xl shadow-sm border border-gray-200 p-12 text-center">
              ...
              <p className="text-gray-600 text-sm">
                {tab === 'matches'
                  ? 'Create a saved search and we will notify you when a matching profile appears.'
                  : tab === 'favorites'
                    ? 'Favorite models to get alerts when they post new photos, stories or change location.'
              ...
              {tab === 'matches' && (
                ...
                >
                  <SearchIcon className="w-4 h-4" /> Create saved search
                </Link>
              )}
            </div>
          ) : (
